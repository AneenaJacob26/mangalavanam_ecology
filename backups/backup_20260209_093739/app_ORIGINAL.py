...
def create_environmental_satellite_map(raster_path, layer_name, center=[9.9833, 76.2167], 
                                       show_boundaries=True, zoom=16):
    """
    Create folium map with environmental raster overlay on satellite imagery
    Uses EXACT Core/Buffer boundaries from GeoJSON files
    
    Args:
        raster_path: Path to environmental raster (NDVI, NDWI, VIIRS)
        layer_name: Name of the layer for display
        center: Map center coordinates
        show_boundaries: Whether to show actual Core/Buffer boundaries
        zoom: Zoom level (higher = more zoomed in)
    
    Returns:
        folium.Map object with satellite base + environmental overlay + boundaries
    """
    import matplotlib.pyplot as plt
    
    # Create base map starting with satellite view
    m = folium.Map(
        location=center,
        zoom_start=zoom,
    ...
    # Add Google Satellite imagery as DEFAULT base layer
    folium.TileLayer(
    ...
    # Also add OpenStreetMap as alternative
    folium.TileLayer(
        tiles='OpenStreetMap',
        name='Street Map',
        overlay=False,
    ...
    # Load and add environmental raster as overlay
    try:
        with rasterio.open(raster_path) as src:
            ...
            if len(valid_data) > 0:
                ...
                if data_max > data_min:
                    raster_normalized = (raster_data - data_min) / (data_max - data_min)
                else:
                    raster_normalized = np.zeros_like(raster_data)
                ...
                if 'NDVI' in layer_name or 'Vegetation' in layer_name:
                    ...
                    cmap = plt.cm.Blues
                    opacity = 0.5
                elif 'VIIRS' in layer_name or 'Light' in layer_name:
                    cmap = plt.cm.hot
                    opacity = 0.5
                else:
                    cmap = plt.cm.RdYlGn
                ...
                # Convert to RGBA
                rgba_data = cmap(raster_normalized)
                # Set transparency for NaN values
                rgba_data[:, :, 3] = np.where(np.isfinite(raster_data), opacity, 0)
                
                # Add as image overlay
                folium.raster_layers.ImageOverlay(
                    rgba_data,
                    bounds=[[bounds_raster.bottom, bounds_raster.left], 
                           [bounds_raster.top, bounds_raster.right]],
                    opacity=1.0,  # Already handled in RGBA
                    interactive=False,
                    cross_origin=False,
                    name=f"{layer_name} Overlay"
                ).add_to(m)
                
    except Exception as e:
        print(f"Warning: Could not load raster overlay: {e}")
    
    # Add EXACT Core and Buffer boundaries from GeoJSON files
    if show_boundaries:
        try:
            # Load actual boundary files
            core, buffer = load_boundaries()
            
            # Add Buffer zone first (so Core appears on top)
            if buffer is not None:
                folium.GeoJson(
                    buffer,
                    name="🟠 Buffer Zone (500m)",
                    ...
                    tooltip=folium.Tooltip("Buffer Zone - Limited Access Area (5.12 ha)")
                ).add_to(m)
            
            # Add Core sanctuary on top
            if core is not None:
                folium.GeoJson(
                    core,
                    name="🟢 Core Sanctuary",
                    ...
                    tooltip=folium.Tooltip("Core Sanctuary - Strictly Protected (2.74 ha)")
                ).add_to(m)
...
def create_habitat_satellite_map(habitat_data, bounds, center=[9.9833, 76.2167], 
                                 show_boundaries=True, zoom=16):
    """
    Create folium map with habitat suitability overlay on satellite imagery
    Uses EXACT Core/Buffer boundaries from GeoJSON files
    
    Args:
        habitat_data: Numpy array of habitat suitability values (0-1)
        bounds: Rasterio bounds object
        center: Map center coordinates
        show_boundaries: Whether to show actual boundaries
        zoom: Zoom level
    
    Returns:
        folium.Map object with satellite base + habitat overlay + boundaries
    """
    import matplotlib.pyplot as plt
    ...
    # Add habitat suitability overlay
    try:
        # Use RdYlGn colormap for habitat quality
        cmap = plt.cm.RdYlGn
        
        # Normalize habitat data (should already be 0-1)
        habitat_normalized = np.clip(habitat_data, 0, 1)
        
        # Convert to RGBA
        rgba_habitat = cmap(habitat_normalized)
        # Set transparency (make NaN transparent)
        rgba_habitat[:, :, 3] = np.where(np.isfinite(habitat_data), 0.6, 0)
        ...
        if bounds is not None:
            folium.raster_layers.ImageOverlay(
                ...
                interactive=False,
                cross_origin=False,
                name="Habitat Suitability Overlay"
            ).add_to(m)
            
    except Exception as e:
        print(f"Warning: Could not create habitat overlay: {e}")
        ...
        try:
            ...
            if buffer is not None:
                folium.GeoJson(
                    ...
                    tooltip=folium.Tooltip("Buffer Zone (5.12 ha)")
                ).add_to(m)
            ...
            if core is not None:
                folium.GeoJson(
                    ...
                    tooltip=folium.Tooltip("Core Sanctuary (2.74 ha)")
                ).add_to(m)
...
# REPLACE WITH THIS COMPLETE VERSION:
    ...
    st.info("""
    **📍 Interactive satellite maps showing:**
    - 🛰️ Google satellite imagery base layer
    - 🌈 Environmental data overlays (NDVI/NDWI/VIIRS)
    - 🟢 **Green outline** = Core Sanctuary (exact boundaries)
    - 🟠 **Orange dashed outline** = Buffer Zone (exact boundaries)
    - ⬜ Click layer icon (top-right) to toggle Satellite/Street view
    """)
    
    # Layer selector with boundary toggle
    col_selector, col_boundary, col_zoom = st.columns([3, 1, 1])
    ...
    if selected_file.exists():
        # Two-column layout for side-by-side maps
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown(f"#### 🛰️ Satellite: {satellite_layer}")
            st.caption("Environmental data on Google satellite imagery with exact sanctuary boundaries")
            
            # Create environmental overlay map with EXACT boundaries
        ...
        with col2:
            ...
            if data is not None and bounds is not None:
                ...
                st_folium(
                    ...
                    height=450,
                    returned_objects=[],
                    key=f"habitat_model_map_{guild_filter}"
                )
                
                st.markdown("🟢 **Green = Good habitat** | 🔴 **Red = Poor habitat**")
            else:
                st.warning("Habitat data not available for this selection")
        
        # Detailed explanation
        st.markdown("---")
        
        with st.expander("💡 How to Read These Maps", expanded=False):
            st.markdown(f"""
            ### Understanding the Satellite Comparison
            
            **LEFT MAP - Environmental Data:**
            - Shows **{satellite_layer.lower()}** measured by satellites
            - Color indicates environmental conditions
            - Overlaid on actual satellite imagery of Mangalavanam
            
            **RIGHT MAP - Habitat Model:**
            - Shows **predicted bird habitat quality**
            - Learned from environmental data + actual bird observations
            - Green areas = High probability birds live there
            
            **Boundary Lines:**
            - 🟢 **Solid Green** = Core Sanctuary (2.74 ha) - Most protected
            - 🟠 **Dashed Orange** = Buffer Zone (5.12 ha) - Limited access
            - These are the **EXACT** boundaries from sanctuary maps!
            
            **How Our Model Works:**
            1. Satellites measure {satellite_layer.lower()} across entire sanctuary
            2. We combine this with water, lights, and distance data
            3. AI learns pattern: "Where do birds actually get seen?"
            4. Model predicts: "Where SHOULD birds be living?"
            
            **Why Compare Them?**
            - See if high vegetation = high bird presence (usually yes!)
            - Identify areas with good environment BUT no birds (why?)
            - Find degraded areas that could be restored
            """)
        ...
        with col_insight1:
            ...
            if satellite_layer == "NDVI (Vegetation)":
                st.success("""
                **Expected Pattern:**
                - Core sanctuary should show greener colors
                - Buffer edges may show less vegetation
                - If habitat map matches NDVI → Model is working!
                """)
            elif satellite_layer == "NDWI (Water)":
                st.info("""
                **Expected Pattern:**
                - Wetland areas show blue (high water)
                - Wetland guild habitat should match these areas
                - Core may have permanent water bodies
                """)
            else:  # VIIRS
                st.warning("""
                **Expected Pattern:**
                - Buffer edges brighter (near urban)
                - Core darker (less light pollution)
                - Urban guild tolerates bright areas better
                """)
        
        with col_insight2:
            st.markdown("### 📊 What to Look For")
            st.markdown(f"""
            **For {guild_filter} Birds:**
            
            1. **Good Match:**
               - Green environmental areas = Green habitat areas
               - Model learned correctly! ✅
            
            2. **Mismatch - Good environment, poor habitat:**
               - Maybe disturbance/threats present
               - Edge effects reducing suitability
            
            3. **Mismatch - Poor environment, good habitat:**
               - Birds adapting to degraded conditions
               - May indicate resilient species
        ...
        st.error(f"""
        ⚠️ **Satellite layer not found:** `{selected_file.name}`
        
        **Expected location:** `data/raw/rasters/`
        
        **Available layers:**
        - NDVI_2025_core.tif (Vegetation Index)
        - NDWI_2025_core.tif (Water Index)
        - VIIRS_2025_core.tif (Night Lights)
        
        **To fix:** Ensure Phase 1 raster preprocessing completed successfully.
        """)
